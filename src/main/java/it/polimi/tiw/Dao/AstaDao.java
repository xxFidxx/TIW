...
public class AstaDao {
    ...
    public ArrayList<Asta> findAsteByVenditore(Integer venditore_id, int chiusa) throws SQLException {
        String query = "SELECT DISTINCT * FROM asta WHERE (venditore_id = ? AND chiusa = ?) ORDER BY data_fine ASC";
        try (PreparedStatement p = connection.prepareStatement(query)) {
            p.setInt(1, venditore_id);
            p.setInt(2, chiusa);
            try (ResultSet rs = p.executeQuery()) {
    ...
    public int createAsta(Asta asta) throws SQLException {
        String query = """
                INSERT INTO asta (venditore_id, data_inizio, data_fine, prezzo_iniziale,prezzo_attuale, rialzo_minimo, chiusa)
                VALUES (?, ?, ?, ?, ?, ?, ?)
                """;
        try (PreparedStatement p = connection.prepareStatement(query, Statement.RETURN_GENERATED_KEYS)) {
            p.setInt(1, asta.getVenditoreId());
    ...
    public ArrayList<Asta> findAstaByParolaChiave(String parolaChiave, LocalDateTime loginTime) throws SQLException {
        ...
        String query = "SELECT DISTINCT at.id, at.venditore_id, at.data_inizio, at.data_fine, " +
    ...
    public ArrayList<Asta> findAllAsteAperte(LocalDateTime loginTime) throws SQLException {
        ArrayList<Asta> aste = new ArrayList<>();
        String query = "SELECT at.id, at.venditore_id, at.data_inizio, at.data_fine, " +
                "at.prezzo_iniziale, at.prezzo_attuale, at.rialzo_minimo, at.chiusa " +
                "FROM asta at " +
                "WHERE at.chiusa = 0 " +
                "AND at.data_fine > ? " +
    ...
    public ArrayList<Asta> findAllAsteAggiudicate(Integer userId) throws SQLException {
        ArrayList<Asta> aste = new ArrayList<>();
        ...
        String query = "SELECT DISTINCT at.id, at.venditore_id, at.data_inizio, at.data_fine, " +
                ...
                "WHERE (o.utente_id = ? AND o.aggiudicata = 1)";
